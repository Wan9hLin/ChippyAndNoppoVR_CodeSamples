...
public class RoomManager : MonoBehaviourPunCallbacks
{
    ...
    public TextMeshProUGUI OccupancyRateText_FreeRoom;
    ...
    public void OnEnterButtonClicked_Free()
    {
        mapType = MultiplayerVRConstants.MAP_TYPE_VALUE_FREE;
        ExitGames.Client.Photon.Hashtable expectedCustomRoomProperties = new ExitGames.Client.Photon.Hashtable() { { MultiplayerVRConstants.MAP_TYPE_KEY, mapType } };
        PhotonNetwork.JoinRandomRoom(expectedCustomRoomProperties, 0);
    }
    #endregion
        ...
        if (PhotonNetwork.CurrentRoom.CustomProperties.ContainsKey(MultiplayerVRConstants.MAP_TYPE_KEY))
        {
            ...
            if (PhotonNetwork.CurrentRoom.CustomProperties.TryGetValue(MultiplayerVRConstants.MAP_TYPE_KEY,out mapType))
            {
                ...
                if ((string)mapType == MultiplayerVRConstants.MAP_TYPE_VALUE_OFFICE)
                {
                    ...
                    PhotonNetwork.LoadLevel("Outdoor2");
                ...
                else if ((string)mapType == MultiplayerVRConstants.MAP_TYPE_VALUE_FREE)
                {
                    //Load the Free scene
                    PhotonNetwork.LoadLevel("Outdoor3");
                }
            }
        }
    ...
    public override void OnRoomListUpdate(List<RoomInfo> roomList)
    {
        ...
        foreach (RoomInfo room in roomList)
        {
            ...
            }else if (room.Name.Contains(MultiplayerVRConstants.MAP_TYPE_VALUE_OFFICE))
            {
                Debug.Log("Room is a School map. Player count is: " +room.PlayerCount);
                OccupancyRateText_ForSchool.text = room.PlayerCount + " / " + 20;
            }
            else if (room.Name.Contains(MultiplayerVRConstants.MAP_TYPE_VALUE_FREE))
            {
                Debug.Log("Room is a Free map. Player count is: " + room.PlayerCount);
                OccupancyRateText_FreeRoom.text = room.PlayerCount + " / " + 20;
            }
        }
